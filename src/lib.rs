...
pub trait ConfigPlugin: std::fmt::Debug + Send + Sync {
    ...
    fn from_env(&self) -> ConfigResult<Self> where Self: Sized;
...
impl ConfigManager {
    ...
    pub fn generate_config_template(&self, output_path: &str) -> ConfigResult<()> {
        ...
        yaml_content.push_str("# This file shows the resolved configuration values after applying hierarchy:\n");
        yaml_content.push_str("# Priority: Environment Variables > YAML file > Defaults\n");
        yaml_content.push_str("#\n");
        yaml_content.push_str("# Environment Variables:\n");
        yaml_content.push_str("# PostgreSQL: TYL_DATABASE_URL, DATABASE_URL, POSTGRES_URL (full connection string)\n");
...
mod tests {
    ...
    fn test_database_url_priority() {
        ...
        std::env::set_var("DATABASE_URL", "postgresql://user:pass@db-host:5432/production");
        
        let mut config = PostgresConfig::default();
        config.merge_env().unwrap();
        
        assert_eq!(config.connection_url(), "postgresql://user:pass@db-host:5432/production");
        
        // Test TYL_DATABASE_URL takes priority
        std::env::set_var("TYL_DATABASE_URL", "postgresql://tyl:secret@tyl-host:5432/tyldb");
        config.merge_env().unwrap();
        
        assert_eq!(config.connection_url(), "postgresql://tyl:secret@tyl-host:5432/tyldb");
