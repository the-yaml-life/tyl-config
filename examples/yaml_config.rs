...
"#;
    ...
    if let Some(postgres) = config.postgres() {
        ...
        println!("  host: {} (source: {})", 
            postgres.host, 
            if postgres.host == "env-postgres" { "TYL_POSTGRES_HOST env var" } else { "YAML/default" }
        );
        println!("  username: {} (source: {})", 
            postgres.username, 
            if postgres.username == "env-user" { "PGUSER env var" } else { "YAML/default" }
        );
    ...
    if let Some(redis) = config.redis() {
        ...
        println!("  port: {} (source: {})", 
            redis.port, 
            if redis.port == 6380 { "TYL_REDIS_PORT env var" } else { "YAML/default" }
        );
    ...
    println!("\n✅ Generated resolved configuration at: {}", final_template_path);
