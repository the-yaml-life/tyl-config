...
fn basic_usage_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(postgres) = config.postgres() {
        ...
        println!(
            "  Database: {}, Pool Size: {}",
            postgres.database, postgres.pool_size
        );
    }
    ...
    if let Some(redis) = config.redis() {
        ...
        println!(
            "  Database: {}, Pool Size: {}",
            redis.database, redis.pool_size
        );
    }
...
fn environment_config_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(postgres) = config.postgres() {
        ...
        println!(
            "  Pool Size: {} (from TYL_POSTGRES_POOL_SIZE)",
            postgres.pool_size
        );
    }
...
fn url_vs_components_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    std::env::set_var(
        "DATABASE_URL",
        "postgresql://prod_user:secret@prod-host:5432/prod_db",
    );
    ...
    match config_components.validate() {
        Ok(_) => println!("❌ Unexpected: validation should have failed"),
        Err(e) => println!(
            "✅ Config without DATABASE_URL: validation failed as expected: {}",
            e
        ),
    }
